...
valid_caixas = [str(num) for num in range(1, 301)] + ['N/A', 'F/C']
...
def normalize_caixa(caixa_input):
    ...
    if caixa_input.isdigit() and 1 <= int(caixa_input) <= 300:
        return caixa_input
...
def create_tables():
    with get_db() as conn:
        ...
        if cursor.fetchone():
            ...
            cursor.execute('''
                INSERT INTO estoque (codigo, quantidade, caixa, reservado)
                SELECT codigo, quantidade, caixa, reservado FROM estoque_old
            ''')
            # Remover a tabela antiga
            cursor.execute("DROP TABLE estoque_old")
        else:
            # Criar tabela estoque se não existir
            cursor.execute('''
            ...
            ''')
        # Resto do código de create_tables() para outras tabelas
        conn.execute('''
        ...
        try:
            ...
            pass
        try:
            cursor.execute("ALTER TABLE transactions ADD COLUMN classificacao TEXT")
        except sqlite3.OperationalError:
            pass
        try:
            cursor.execute("ALTER TABLE transactions ADD COLUMN observacao TEXT")
        except sqlite3.OperationalError:
            pass
        conn.commit()
...
def login():
    if current_user.is_authenticated:
        # AJUSTE: Adicionado o redirecionamento para 'consulta_ean' se já estiver logado.
        if current_user.username == 'consulta_ean':
            redirect_url = url_for('consulta_ean')
        elif current_user.role == 'consulta':
            redirect_url = url_for('consulta')
        elif current_user.role == 'conferente':
            redirect_url = url_for('conferente')
        elif current_user.role == 'impressao':
            redirect_url = url_for('image_index')
        elif current_user.role == 'pedidos':
            redirect_url = url_for('pedidos')
        elif current_user.role == 'producao':
            redirect_url = url_for('producao')
        else:
            redirect_url = url_for('index')  # Admin and others default to 'index'
    ...
    if request.method == 'POST':
        ...
        with get_db() as conn:
            ...
            # AJUSTE: A lógica para usuários sem senha foi refinada para garantir o redirecionamento correto.
            # O usuário 'consulta_ean' tem a role 'conferente', então ele entra neste bloco.
            if username in ['consulta', 'conferente', 'consulta_ean'] and (not user[2] or user[2] == ''):
                user_obj = User(user[0], user[1], user[3])
                login_user(user_obj)
                
                # Define o redirecionamento específico para cada usuário sem senha
                if username == 'consulta_ean':
                    redirect_url = url_for('consulta_ean')
                elif username == 'conferente':
                    redirect_url = url_for('conferente')
                else: # 'consulta'
                    redirect_url = url_for('consulta')
...
def get_loja_eans(loja_id):
    """Retorna todos os EANs de uma loja, com filtro estrito por sufixo de SKU."""
    try:
        ...
        # Se a busca estiver vazia, não retorna nada para manter a tela limpa.
        if not sku_filter:
        ...
        # --- INÍCIO DA CORREÇÃO ---
        # Constrói a consulta base e os parâmetros
        query = """
            SELECT ed.id, ed.sku, ed.ean, els.checked, els.checked_by
            FROM ean_data ed
            LEFT JOIN ean_loja_status els ON ed.id = els.ean_data_id AND els.loja_id = ?
        """
        params = [loja_id]
        conditions = []

        # 1. Adiciona o filtro de busca do usuário
        conditions.append("ed.sku LIKE ?")
        params.append(f"%{sku_filter}%")

        # 2. Adiciona o filtro ESTRITO de loja com base no sufixo do SKU
        if loja_id == 'loja1':
            # Loja 1: SKUs que NÃO terminam com -F, -P, ou -V.
            conditions.append("ed.sku NOT LIKE '%-F' AND ed.sku NOT LIKE '%-P' AND ed.sku NOT LIKE '%-V'")
        elif loja_id == 'loja2':
            # Loja 2: SKUs que DEVEM terminar com -F.
            conditions.append("ed.sku LIKE '%-F'")
        elif loja_id == 'loja3':
            # Loja 3: SKUs que DEVEM terminar com -P.
            conditions.append("ed.sku LIKE '%-P'")
        elif loja_id == 'loja4':
            # Loja 4: SKUs que DEVEM terminar com -V.
            conditions.append("ed.sku LIKE '%-V'")
        
        # Junta todas as condições na query final
        if conditions:
            query += " WHERE " + " AND ".join(conditions)
        # --- FIM DA CORREÇÃO ---
        ...
        SUBLOJAS = ['mercado', 'site', 'magalu', 'shopee', 'shein']
        
        eans_agrupados = {}
        for row in rows:
            chave = f"{row['sku']}|{row['ean']}"
            if chave not in eans_agrupados:
                eans_agrupados[chave] = {
                    'id': row['id'],
                    'sku': row['sku'],
                    'ean': row['ean'],
                    'checks': {subloja: {'checked': False, 'checked_by': None} for subloja in SUBLOJAS}
                }

            if row['checked_by']:
                partes = row['checked_by'].split(';')
                for parte in partes:
                    if ':' in parte:
                        subloja, usuario = parte.split(':', 1)
                        if subloja in SUBLOJAS:
                            eans_agrupados[chave]['checks'][subloja] = {
                                'checked': True,
                                'checked_by': usuario
                            }
                            
        return jsonify({'success': True, 'data': list(eans_agrupados.values())})
...
@login_required
def update_status():
    """Atualiza o status de verificação para uma subloja específica."""
    try:
        data = request.get_json()
        
        # Adiciona 'subloja_id' aos campos obrigatórios
        required_fields = ['ean_data_id', 'loja_id', 'subloja_id', 'checked']
        ...
        # Pega o status atual
        cursor.execute("SELECT checked_by FROM ean_loja_status WHERE ean_data_id = ? AND loja_id = ?", (ean_data_id, loja_id))
        row = cursor.fetchone()
        
        current_checked_by = row['checked_by'] if row and row['checked_by'] else ""
        
        # Decodifica os checks existentes em um dicionário
        checks = {}
        if current_checked_by:
            partes = current_checked_by.split(';')
            for parte in partes:
                if ':' in parte:
                    sub, user = parte.split(':', 1)
                    checks[sub] = user

        # Atualiza o dicionário com a nova informação
        if is_checked:
            checks[subloja_id] = current_user.username
        else:
            if subloja_id in checks:
                del checks[subloja_id]

        # Reconstrói a string para o campo checked_by
        new_checked_by = ";".join([f"{sub}:{user}" for sub, user in checks.items()])
        
        # Atualiza o campo 'checked' para 1 se houver qualquer check, ou 0 se estiver vazio
        new_checked_status = 1 if checks else 0
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S') if new_checked_status else None
        ...
        """, (new_checked_status, current_time, new_checked_by, ean_data_id, loja_id))
        
        # Se não houver um registro, cria um novo (caso de primeiro check)
        if cursor.rowcount == 0:
            cursor.execute("""
                INSERT INTO ean_loja_status (ean_data_id, loja_id, checked, checked_date, checked_by)
                VALUES (?, ?, ?, ?, ?)
            """, (ean_data_id, loja_id, new_checked_status, current_time, new_checked_by))
...
def consulta():
    ...
    prefix_filter = request.args.get('prefix', '').strip().upper()
    tema_filter = request.args.get('tema', '').strip().upper()
    caixa_filter = request.args.get('caixa', '').strip().upper()

    with get_db() as conn:
        cursor = conn.cursor()
        # Consulta principal para itens por caixa
        query = "SELECT id, codigo, quantidade, COALESCE(caixa, 'N/A') as caixa, reservado FROM estoque WHERE quantidade > 0"
        params = []
        conditions = []
        ...
        if tema_filter and len(tema_filter) == 2:
            ...
            params.append(f"{prefix_filter}{tema_filter}%")
        if caixa_filter:
            normalized_caixa = normalize_caixa(caixa_filter)
            if normalized_caixa:
                conditions.append("caixa = ?")
                params.append(normalized_caixa)
            else:
                print(f'[Server] Caixa inválida fornecida: {caixa_filter}')
                flash('Caixa inválida. Use números de 1 a 300, N/A ou F/C.', 'error')

        if conditions:
            query += " AND " + " AND ".join(conditions)
        query += " ORDER BY codigo ASC, caixa ASC"

        cursor.execute(query, params)
        items = cursor.fetchall()

        # Consulta para totais por SKU
        total_query = "SELECT codigo, SUM(quantidade) as total_quantidade FROM estoque WHERE quantidade > 0"
        total_params = []
        total_conditions = []

        if prefix_filter and prefix_filter in valid_prefixes:
            total_conditions.append("codigo LIKE ?")
            total_params.append(f"{prefix_filter}%")
        if tema_filter and len(tema_filter) == 2:
            total_conditions.append("codigo LIKE ?")
            total_params.append(f"{prefix_filter}{tema_filter}%")

        if total_conditions:
            total_query += " AND " + " AND ".join(total_conditions)
        total_query += " GROUP BY codigo"
        cursor.execute(total_query, total_params)
        totals = {row['codigo']: row['total_quantidade'] for row in cursor.fetchall()}
    ...
    for item in items:
        ...
        for ext in ['jpg', 'jpeg', 'png', 'gif']:
            ...
            if os.path.exists(file_path):
                image_url = url_for('get_apr_image', sku=sku)
                break
        sku_images.append({
            'id': item['id'],
            'codigo': sku,
            ...
            'total_quantidade': totals.get(sku, item['quantidade'])
        })

    print(f'[Server] Acesso à consulta: Usuário {current_user.username}, Itens={len(sku_images)}, Caixa_Filter={caixa_filter}')
    return render_template('consulta.html', sku_images=sku_images, valid_prefixes=valid_prefixes, prefix_filter=prefix_filter, tema_filter=tema_filter, caixa_filter=caixa_filter)
...
def add_remove_item():
    ...
    if normalized_caixa is None and caixa not in ['N/A', '']:
        ...
        return jsonify({'success': False, 'error': 'Caixa inválida. Use números de 1 a 300, N/A ou F/C.'}), 400
    ...
    with get_db() as conn:
        ...
        try:
            if action == 'add':
                # Verificar se existe uma entrada para o SKU com a mesma caixa
                cursor.execute("SELECT id, quantidade FROM estoque WHERE codigo = ? AND caixa = ?", (sku, caixa))
                item = cursor.fetchone()
                if item:
                    # Atualizar quantidade se a caixa for a mesma
                    cursor.execute("UPDATE estoque SET quantidade = quantidade + ? WHERE id = ?",
                                  (quantity, item['id']))
                    print(f'[Server] Quantidade atualizada: SKU={sku}, Caixa={caixa}, Quantidade adicionada={quantity}')
                else:
                    # Criar nova entrada para o SKU com a nova caixa
                    cursor.execute("INSERT INTO estoque (codigo, quantidade, caixa) VALUES (?, ?, ?)",
                                  (sku, quantity, caixa))
                    print(f'[Server] Nova entrada criada: SKU={sku}, Caixa={caixa}, Quantidade={quantity}')
                # Registrar transação
                cursor.execute("INSERT INTO transactions (sku, transaction_type, quantity, date, caixa, classificacao, observacao) VALUES (?, ?, ?, ?, ?, ?, ?)",
                              (sku, 'entrada', quantity, current_date, caixa, classificacao, observacao))
                message = f'{quantity} unidade(s) de {sku} adicionada(s) com sucesso na caixa {caixa}.'
            else:  # action == 'remove'
                # Verificar se existe uma entrada para o SKU com a mesma caixa
                cursor.execute("SELECT id, quantidade FROM estoque WHERE codigo = ? AND caixa = ?", (sku, caixa))
                item = cursor.fetchone()
                if not item:
                    print(f'[Server] Erro: SKU={sku} não encontrado na caixa={caixa}')
                    return jsonify({'success': False, 'error': f'SKU {sku} não encontrado na caixa {caixa}.'}), 400
                if item['quantidade'] < quantity:
                    print(f'[Server] Erro: Quantidade insuficiente para SKU={sku}, Caixa={caixa}, Disponível={item["quantidade"]}')
                    return jsonify({'success': False, 'error': f'Quantidade insuficiente em estoque para {sku} na caixa {caixa}.'}), 400
                # Atualizar quantidade
                new_quantity = item['quantidade'] - quantity
                cursor.execute("UPDATE estoque SET quantidade = ? WHERE id = ?",
                              (new_quantity, item['id']))
                print(f'[Server] Quantidade atualizada: SKU={sku}, Caixa={caixa}, Nova quantidade={new_quantity}')
                # Registrar transação
                cursor.execute("INSERT INTO transactions (sku, transaction_type, quantity, date, caixa, classificacao, observacao) VALUES (?, ?, ?, ?, ?, ?, ?)",
                              (sku, 'saida', quantity, current_date, caixa, classificacao, observacao))
                message = f'{quantity} unidade(s) de {sku} removida(s) com sucesso da caixa {caixa}.'
            conn.commit()
            print(f'[Server] Ação concluída: {message}')
            return jsonify({'success': True, 'message': message})
        except sqlite3.Error as e:
            conn.rollback()
            print(f'[Server] Erro ao processar {action} para SKU={sku}, Caixa={caixa}: {str(e)}')
...
def bulk_add_remove():
    ...
    current_date = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
    classificacoes = request.form.getlist("classificacao")
    observacao_outros = request.form.get("outros_observacao", "").strip()
    classificacao_str = ", ".join(classificacoes) if classificacoes else None
    ...
    with get_db() as conn:
        ...
        for index, line in enumerate(bulk_items.split('\n'), 1):
            ...
            try:
                ...
                # Define a caixa, tratando o caso de não ser fornecida
                caixa = 'N/A'
                if len(parts) >= 3 and parts[2].strip():
                    caixa_input = parts[2].strip().upper()
                    normalized_caixa = normalize_caixa(caixa_input)
                    if normalized_caixa is None:
                        error_messages.append(f'Linha {index}: Caixa inválida para {sku}. Use 1-300, N/A ou F/C.')
                        continue
                    caixa = normalized_caixa

                sku_prefixo = sku[:2]
                if sku_prefixo not in (skus_que_precisam_caixa_prefixos + skus_sem_caixa):
                    error_messages.append(f'Linha {index}: SKU {sku} possui prefixo inválido.')
                    continue

                if action == 'bulk_add':
                    # --- CORREÇÃO PRINCIPAL ---
                    # Procura por um item com o mesmo SKU e a MESMA CAIXA.
                    cursor.execute("SELECT id, quantidade FROM estoque WHERE codigo = ? AND caixa = ?", (sku, caixa))
                    item = cursor.fetchone()

                    if item:
                        # Se o item já existe na mesma caixa, SOMA a quantidade.
                        cursor.execute("UPDATE estoque SET quantidade = quantidade + ? WHERE id = ?", (quantity, item['id']))
                        print(f'[Server] Quantidade somada: SKU={sku}, Caixa={caixa}, Quantidade Adicionada={quantity}')
                    else:
                        # Se não existe nessa caixa, cria uma nova entrada para o SKU naquela caixa.
                        if sku_prefixo in skus_que_precisam_caixa_prefixos and caixa == 'N/A':
                            error_messages.append(f'Linha {index}: SKU {sku} requer uma caixa válida (1-300 ou F/C).')
                            continue
                        cursor.execute("INSERT INTO estoque (codigo, quantidade, caixa) VALUES (?, ?, ?)", (sku, quantity, caixa))
                        print(f'[Server] Nova entrada criada: SKU={sku}, Caixa={caixa}, Quantidade={quantity}')

                    # Registra a transação
                    cursor.execute("""
                        INSERT INTO transactions (sku, transaction_type, quantity, date, caixa, classificacao, observacao)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (sku, 'entrada', quantity, current_date, caixa, classificacao_str, observacao_outros))
                    success_messages.append(f'Linha {index}: Adicionado {quantity} de {sku} na caixa {caixa}.')

                elif action == 'bulk_remove':
                    # A lógica de remoção também deve ser específica da caixa.
                    cursor.execute("SELECT id, quantidade FROM estoque WHERE codigo = ? AND caixa = ?", (sku, caixa))
                    item = cursor.fetchone()

                    if not item:
                        error_messages.append(f'Linha {index}: SKU {sku} não encontrado na caixa {caixa}.')
                        continue
                    if item['quantidade'] < quantity:
                        error_messages.append(f'Linha {index}: Quantidade insuficiente para {sku} na caixa {caixa}. Disponível: {item["quantidade"]}.')
                        continue

                    # Subtrai a quantidade do registro correto.
                    new_quantity = item['quantidade'] - quantity
                    cursor.execute("UPDATE estoque SET quantidade = ? WHERE id = ?", (new_quantity, item['id']))

                    # Registra a transação
                    cursor.execute("""
                        INSERT INTO transactions (sku, transaction_type, quantity, date, caixa, classificacao, observacao)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (sku, 'saida', quantity, current_date, caixa, classificacao_str, observacao_outros))

                    # Se a quantidade zerar, remove o registro daquela caixa específica.
                    if new_quantity <= 0:
                        cursor.execute("DELETE FROM estoque WHERE id = ?", (item['id'],))
                    
                    success_messages.append(f'Linha {index}: Removido {quantity} de {sku} da caixa {caixa}.')
...
@app.route('/buscar_por_tema', methods=['GET'])
@login_required
def buscar_por_tema():
    try:
        ...
        with get_db() as conn:
            cursor = conn.cursor()
            query = "SELECT codigo, quantidade, COALESCE(caixa, 'N/A') as caixa, reservado FROM estoque WHERE "
            params = []
            conditions = []

            if termos:
                # Busca por tema: assume que termos são palavras-chave amplas
                conditions.append(" OR ".join(["codigo LIKE ?" for _ in termos]))
                params.extend([f"%{termo}%" for termo in termos])
            else:
                # Se não houver termos, buscar apenas por caixa (se fornecida)
                conditions.append("1=1")

            # Adiciona filtro de caixa, se fornecido
            if caixa_filter:
                normalized_caixa = normalize_caixa(caixa_filter)
                if normalized_caixa:
                    conditions.append("caixa = ?")
                    ...
                    print(f'[Server] Caixa inválida fornecida: {caixa_filter} por {current_user.username}')
                    return jsonify({'success': False, 'items': [], 'error': 'Caixa inválida. Use números de 1 a 300, N/A ou F/C.'}), 400
            ...
            for row in cursor.fetchall():
                if row['codigo'] not in [item['codigo'] for item in items]:  # Evita duplicatas
                    ...
                    print(f'[Server] Encontrado SKU: {row["codigo"]} (caixa: {row["caixa"]}) para busca por tema (role: {current_user.role})')

        print(f'[Server] Busca por tema: {input_raw}, Caixa: {caixa_filter}, Resultados: {len(items)} para {current_user.username} (role: {current_user.role})')
        return jsonify({'success': True, 'items': items})

    except Exception as e:
        print(f'[Server] Erro na busca por tema para {current_user.username} (role: {current_user.role}): {str(e)}')
        return jsonify({'success': False, 'items': [], 'error': 'Erro interno no servidor.'}), 500    
    
@app.route('/get_caixa_count')
@login_required
def get_caixa_count():
    caixa = request.args.get('caixa')
    if not caixa:
        return jsonify({'success': False, 'error': 'Caixa não informada'})

    try:
        conn = sqlite3.connect('estoque.db')
        cursor = conn.cursor()
        cursor.execute("SELECT SUM(quantidade) FROM estoque WHERE caixa = ?", (caixa,))
        result = cursor.fetchone()
        conn.close()
...
def search_item():
    try:
        input_raw = request.args.get('sku', '').upper()
        caixa_filter_from_field = request.args.get('caixa', '').strip().upper() # Campo de filtro de caixa separado
        
        termos = [t.strip() for t in input_raw.split(',') if t.strip()]
        
        # Se não houver termos de busca no campo principal, retorna vazio.
        if not termos:
            print(f'[Server] Nenhum termo de busca fornecido por {current_user.username}')
            return jsonify({'items': []})

        items = []
        
        # Identificar termos de caixa (ex.: "CX 1", "F/C") e termos de SKU
        caixa_termos_from_input = []
        sku_termos = []
        for termo in termos:
            termo_normalized = termo.replace(' ', '')
            if termo_normalized.startswith('CX') or termo_normalized == 'F/C':
                if termo_normalized.startswith('CX'):
                    caixa_num = termo_normalized[2:].strip()
                    if caixa_num.isdigit() and 1 <= int(caixa_num) <= 300:
                        caixa_termos_from_input.append(caixa_num)
                    elif caixa_num == '':
                        caixa_termos_from_input.append('N/A')
                    else:
                        return jsonify({'items': [], 'error': f'Caixa inválida: {termo}. Use números de 1 a 300, N/A ou F/C.'}), 400
                else:
                    caixa_termos_from_input.append('F/C')
            else:
                sku_termos.append(termo)

        # Define o filtro de caixa final a ser usado na consulta
        final_caixa_filter = None
        if caixa_filter_from_field:
            # Se o campo de filtro de caixa foi usado, ele tem prioridade.
            if caixa_termos_from_input:
                return jsonify({'items': [], 'error': 'Conflito: Use o campo de busca para SKUs ou para caixas, mas não ambos ao mesmo tempo com o filtro de caixa.'}), 400
            final_caixa_filter = caixa_filter_from_field
        elif caixa_termos_from_input:
            # Se não, usa os termos de caixa do campo de busca principal.
            final_caixa_filter = caixa_termos_from_input[0] # Usa o primeiro termo de caixa encontrado

        with get_db() as conn:
            cursor = conn.cursor()
            
            # Constrói uma única consulta SQL para todos os termos
            query = "SELECT codigo, quantidade, COALESCE(caixa, 'N/A') as caixa, reservado FROM estoque WHERE quantidade > 0"
            params = []
            conditions = []

            if sku_termos:
                # Cria uma condição OR para todos os termos de SKU
                sku_conditions = []
                for termo in sku_termos:
                    is_complete_sku = len(termo) >= 5 or any(termo.startswith(p) for p in ["PV", "PH", "FF", "FH", "RV", "PR", "PVSE", "PCRV", "KCMD"])
                    if is_complete_sku:
                        if current_user.role == 'impressao':
                            sku_conditions.append("codigo = ?")
                            params.append(termo)
                        else:
                            sku_conditions.append("(codigo = ? OR codigo LIKE ?)")
                            params.extend([termo, f"{termo}%"])
                    else:
                        sku_conditions.append("codigo LIKE ?")
                        params.append(f"%{termo}%")
                conditions.append(f"({ ' OR '.join(sku_conditions) })")

            if final_caixa_filter:
                normalized_caixa = normalize_caixa(final_caixa_filter)
                if normalized_caixa:
                    conditions.append("caixa = ?")
                    params.append(normalized_caixa)
                else:
                    return jsonify({'items': [], 'error': 'Caixa inválida. Use números de 1 a 300, N/A ou F/C.'}), 400
            ...
            print(f'[Server] Executando consulta: {query} com parâmetros {params}')
            cursor.execute(query, params)
            
            # Adiciona todos os resultados encontrados à lista de itens
            for row in cursor.fetchall():
                items.append({
        ...
        print(f'[Server] Busca por: "{input_raw}", Resultados: {len(items)} para {current_user.username}')
        return jsonify({'items': items})

    except Exception as e:
        print(f'[Server] Erro na busca de itens para {current_user.username} (role: {current_user.role}): {str(e)}')
        return jsonify({'items': [], 'error': 'Erro interno no servidor.'}), 500
...
def all_sku_transactions():
    ...
    with get_db() as conn:
        ...
        # Busca os 5 itens com mais saídas (lógica existente)
        cursor.execute("""
        ...
        # Busca todas as transações com base no filtro de SKU
        if termos:
            query_conditions = " OR ".join(["sku LIKE ?"] * len(termos))
            query_params = [f"%{termo}%" for termo in termos]
            sql = f"""
                SELECT sku, transaction_type, quantity, date, caixa, classificacao, observacao
                FROM transactions
        ...
        # Processa as transações e calcula os totais corretos
        for trans in all_trans:
            sku = trans['sku']
            if sku not in sku_transactions:
                # --- CORREÇÃO PRINCIPAL AQUI ---
                # Calcula a SOMA das quantidades para este SKU em TODAS as caixas.
                cursor.execute("SELECT SUM(quantidade) as total_quantity FROM estoque WHERE codigo = ?", (sku,))
                total_item = cursor.fetchone()
                total_quantity = total_item['total_quantity'] if total_item and total_item['total_quantity'] is not None else 0
                ...
                sku_transactions[sku] = {
                    'transactions': [],
                    'quantity': total_quantity,  # Usa a quantidade somada e correta
                    'image_url': url_for('get_image', sku=sku) if image_exists else None
                }
            
            # Adiciona a transação ao seu respectivo SKU
    ...
    # Adiciona a lista de classificações para os filtros do frontend
    for sku, data in sku_transactions.items():
        classificacoes = {t['classificacao'] for t in data['transactions'] if t.get('classificacao')}
        data['classificacoes'] = list(classificacoes)
...
def download_all_sku_transactions():
    ...
    with get_db() as conn:
        ...
        # Define a base da query de transações
        base_query = """
            SELECT sku, transaction_type, quantity, date, caixa, classificacao, observacao
            FROM transactions
        """
        ...
        # Adiciona filtros se houver termos de busca
        if termos:
            conditions = " OR ".join(["sku LIKE ?"] * len(termos))
            base_query += f" WHERE {conditions}"
            params.extend([f"%{termo}%" for termo in termos])
        
        base_query += " ORDER BY sku, date_to_sortable(date)"
        
        cursor.execute(base_query, params)
        all_trans = cursor.fetchall()

        # Agrupa as transações por SKU
        for trans in all_trans:
            sku = trans['sku']
            if sku not in data_por_sku:
                # --- CORREÇÃO PRINCIPAL AQUI ---
                # Para cada SKU, faz uma nova consulta para SOMAR a quantidade total do estoque.
                cursor.execute("SELECT SUM(quantidade) as total_quantity FROM estoque WHERE codigo = ?", (sku,))
                total_item = cursor.fetchone()
                total_quantity = total_item['total_quantity'] if total_item and total_item['total_quantity'] is not None else 0
                ...
                data_por_sku[sku] = {
                    'transactions': [],
                    'quantity': total_quantity,  # Armazena a quantidade total correta
                }
            data_por_sku[sku]['transactions'].append(trans)

    # --- GERAÇÃO DO PDF (sem alterações, pois agora usa os dados corretos) ---
    buffer = BytesIO()
    ...
    for sku, data in sorted(data_por_sku.items()):
        ...
        # Esta linha agora usará a 'quantity' correta que foi calculada acima
        ...
        p.drawString(margin_left + 180, y, "QUANTIDADE / CLASSIFICAÇÃO / OBSERVAÇÃO".ljust(70))
        p.drawString(margin_left + 500, y, "CAIXA")
        y -= 15
        ...
        for trans in sorted(data['transactions'], key=lambda x: sqlite_date_to_sortable(x['date'])):
            ...
            classificacao = trans["classificacao"] or ""
            observacao = trans["observacao"] or ""
            ...
            detalhe = quantidade
            if classificacao:
                detalhe += f" | {classificacao}"
            if observacao:
                detalhe += f" | {observacao}"

            p.drawString(margin_left + 0, y, tipo_str.ljust(12)[:12])
            p.drawString(margin_left + 90, y, data_str.ljust(10)[:10])
            p.drawString(margin_left + 180, y, detalhe.ljust(70)[:70])
            p.drawString(margin_left + 500, y, caixa[:10])
...
# Junta as classificações em uma string separada por vírgula
        classificacao_str = ", ".join(classificacoes) if classificacoes else None
        success_count = 0
        error_messages = []
...
def edit_item():
    ...
    sku = request.form.get('barcode')
    try:
        nova_quantidade = int(request.form.get('quantity'))
        nova_caixa = request.form.get('caixa') or 'N/A'
        # --- AJUSTE PRINCIPAL: Recebe a caixa original para identificar a linha ---
        caixa_original = request.form.get('original_caixa')
    except (ValueError, TypeError):
        return jsonify({'success': False, 'error': 'Valores inválidos.'}), 400

    if not sku or not caixa_original:
        return jsonify({'success': False, 'error': 'SKU ou caixa original não fornecidos.'}), 400

    with get_db() as conn:
        cursor = conn.cursor()
        
        # Busca o item usando a combinação de SKU e a caixa original
        cursor.execute("SELECT id, quantidade FROM estoque WHERE codigo = ? AND caixa = ?", (sku, caixa_original))
        item = cursor.fetchone()

        if not item:
            return jsonify({'success': False, 'error': f'Item {sku} na caixa {caixa_original} não encontrado.'}), 404
        ...
        # Atualiza o registro específico usando o ID único que encontramos
        cursor.execute("""
            UPDATE estoque SET quantidade = ?, caixa = ? WHERE id = ?
        """, (nova_quantidade, nova_caixa, item_id))

        # Lógica para registrar a transação (não precisa de alteração)
        if nova_quantidade != quantidade_antiga:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (sku, tipo, diferenca, data, nova_caixa))
        elif nova_caixa != caixa_original:
            cursor.execute("""
                INSERT INTO transactions (sku, transaction_type, quantity, date, caixa)
                VALUES (?, ?, ?, ?, ?)
            """, (sku, 'transferencia', 0, data, nova_caixa))

        conn.commit()

    return jsonify({'success': True, 'message': f'Item {sku} atualizado com sucesso!'})
...
# Rota no servidor Flask para processar o cancelamento
@app.route('/cancel_orders', methods=['POST'])
@login_required
def cancel_orders():
    # Garante que apenas usuários autorizados possam cancelar
    if current_user.role not in ['pedidos', 'admin']:
        return jsonify({'success': False, 'message': 'Acesso negado.'}), 403
    ...
    conn = get_pedidos_ml_db() # Conecta ao banco de dados de pedidos
    try:
        cursor = conn.cursor()
        checked_date = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
        ...
        for order_id in order_ids:
            ...
            cursor.execute("""
                UPDATE pedidos 
                SET 
                    checked = 1,              -- Marca como checado para mover para o histórico
                    status = 'cancelado',     -- Define o status como 'cancelado'
                    checked_date = ?,         -- Grava a data/hora do cancelamento
                    producao = 'N/A',         -- Define produção como N/A
                    impressora = 'N/A'        -- Define impressora como N/A
                WHERE id = ? AND checked = 0
            """, (checked_date, order_id))

        conn.commit()
        return jsonify({'success': True, 'message': 'Pedidos cancelados com sucesso.'})
...
def extract_vc_orders(data):
    ...
    if loja not in ['Whatsapp', 'Shein', 'Site', 'Magalu', 'Shopee', 'Mercado livre']:
        return []
...
def process_orders_ml_producao():
    ...
    try:
        ...
        print(f'[Server] Erro ao processar JSON: {str(e)}')
        ...
        for order in orders:
            ...
            if (order['order_id'], order['sku']) in existing_orders:
                ...
                inserted_orders.append({
                    ...
                    'status': order['status'],
                    'info_data': json.dumps(info_data) if info_data else None
                })
                continue
...
def check_orders_producao():
    ...
    data = request.get_json()
    order_ids = data.get('order_ids', [])
    action = data.get('action', 'check')  # Ação padrão é 'check'

    if not order_ids:
        return jsonify({'success': False, 'message': 'Nenhum ID de pedido fornecido.'}), 400

    # Define o status com base na ação recebida
    if action == 'estoque':
        producao_status = 'Estoque'
        status_update = None
    elif action == 'costura':
        producao_status = 'Costura'
        status_update = None
    elif action == 'cancel':
        producao_status = 'N/A'
        status_update = 'cancelado' # Status a ser salvo no banco
    else: # Ação 'check'
        producao_status = ''
        status_update = None
    ...
    try:
        ...
        for order_id in order_ids:
            # Pega o status atual para não sobrescrever se não for um cancelamento
            cursor.execute("SELECT status FROM pedidos WHERE id = ?", (order_id,))
            current_status = cursor.fetchone()[0]

            # Atualiza o pedido no banco de dados
            cursor.execute("""
                UPDATE pedidos 
                SET checked = 1, checked_date = ?, producao = ?, status = ?
                WHERE id = ?
            """, (checked_date, producao_status, status_update if status_update else current_status, order_id))

        conn.commit()
        return jsonify({
            'success': True,
            'message': f'Pedidos marcados como {action} com sucesso.'
        })
    except sqlite3.Error as e:
        conn.rollback()
        return jsonify({'success': False, 'message': f'Erro ao processar pedidos: {str(e)}'}), 500
